  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded p-4 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-sm font-bold text-slate-100 uppercase tracking-wider">Failed Payment Recovery Queue</h2>
          <p className="text-xs text-slate-400">Failed payment episodes evaluated by O1 for safe economic action selection.</p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded p-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-slate-950 border border-slate-800 rounded px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-indigo-500"
          >
              ...
              {loading ? (
                ...
                filteredEvents.map((evt) => (
                  <tr key={evt.payment_id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-3.5 py-2.5 font-mono font-semibold text-indigo-400">
                      {evt.payment_id}
                    </td>
                    <td className="px-3.5 py-2.5 font-mono font-bold text-slate-100">
                      ₹{evt.amount.toFixed(2)}
                    </td>
                    <td className="px-3.5 py-2.5 capitalize text-slate-300">
                      {evt.payment_method.replace('_', ' ')}
                    </td>
                    <td className="px-3.5 py-2.5">
                      <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-slate-950 text-slate-300 border border-slate-800">
                        {formatCategory(evt.failure_category)}
                      </span>
                    </td>
                    <td className="px-3.5 py-2.5 font-mono text-[11px] text-slate-400 truncate max-w-[180px]">
                      {evt.failure_code}
                    </td>
                    ...
                    <td className="px-3.5 py-2.5 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-indigo-300 border border-slate-700 text-xs font-medium transition-colors"
                      >
